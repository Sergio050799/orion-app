...
const SUMMARY_FILE = path.join(SUMMARY_DIR, "summary_financial.json");
const TMP_FILE = path.join(SUMMARY_DIR, `summary_financial_${crypto.randomUUID()}.tmp`);
...
async function writeSummaryAtomic(summaryObj: any) {
    await ensureDir();
...
export async function GET() {
    try {
        ...
        console.error("[ORION API] GET Financial History Error:", e);
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === "UPSERT_RECORD") {
            ...
            summary.records = summary.records.filter((r: any) => !idsToDelete.includes(r.id));
        }
        else {
        ...
        console.error("[ORION API] POST Financial History Error:", e);
        return NextResponse.json({ ok: false, error: e.message }, { status: 500 });
    }
}
